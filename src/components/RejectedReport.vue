              ...
              <template v-if="showQraatLevelSelect">
                <td class="level-number">
                  <span class="level-badge">{{
                    getParticipantQraatTitle(item, competition.value)
                  }}</span>
                </td>
              </template>
              <template v-else>
...
console.log("sdcsd" , competition.value)
...
const qraatLevelItems = computed(() => buildQraatLevelItems(competition.value));
...
const filteredParticipants = computed(() => {
  ...
  if (selectedLevel.value) {
    if (effectiveCompetitionCategory.value === "qraat") {
      filtered = filtered.filter(
        (participant) =>
          getParticipantQraatLevelId(participant) === selectedLevel.value
      );
    } else {
      filtered = filtered.filter(
...
const loadParticipants = async () => {
  ...
  try {
    ...
    // Load participants
    const response = await getStudentsByStatus("rejected", competitionId.value);
...
function handlePrint() {
  ...
  const printRows = filteredParticipants.value.map((p, index) => ({
    ...
    qraatTitle: showQraatLevelSelect.value
